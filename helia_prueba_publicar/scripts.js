...
async function envio_por_ipfs(motivo, mensaje){
    let palabras_clave = document.querySelector("textarea.textarea_palabrasclave").value;
        ...
        let chunks = [];
        for await (const chunk of fs.cat(cid.toString())) {
            chunks.push(chunk);
        }
        ...
        let string = "Comparte las palabras y el código con quien recibe (en este mismo orden):" + "*SEPARAR*" + palabras_clave + " " + cid.toString()// creo una string compleja para pasar datos
